...
## 职责
DAV 的 AE2 存储适配器。将 `DigitalAssetVaultBlockEntity` 内的物品 map 暴露为网格可访问的 `MEStorage`：insert/extract/getAvailableStacks/isPreferredStorageFor。

## 边界检查
边界健康。所有状态变更委托给 BlockEntity；本类不持久化、不决策 route/zone。
