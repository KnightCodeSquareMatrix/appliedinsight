...
| **职责** | 调用 `Ae2StorageAnalyzer` 分析网格，将 `StorageAnalyzerReport` 序列化为 JSON 写入 `dumps/appliedstoragesorter/storage-analysis-*.json`；同时输出 `latest` 版本供前端动态读取 |
| **关键方法** | `dump(Ae2GridTargetResult)` → `SorterStorageAnalysisDumpResult` |
| **输出文件** | `dumps/appliedstoragesorter/storage-analysis-{slug}-{timestamp}.json` + `storage-analysis-{slug}-latest.json` |
