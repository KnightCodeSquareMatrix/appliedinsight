#!/usr/bin/env python3
"""Background texture generation from pixel-position maps and theme palettes."""
import re
import json
import struct
import zlib
...
def parse_theme_colors(theme_entry):
    """Convert hex strings in theme to (R,G,B,A) tuples."""
    colors = {}
...
class BackgroundPixelMap:
    """Parsed pixel-position file with color analysis."""
    ...
    def classify_by_luminance(self):
        """Auto-classify colors into 4 background roles by largest luminance gaps."""
    ...
    def show_colors(self):
        """Print color statistics for manual mapping."""
        print(f"\n=== Pixel map: {self.path.name} ===")
        print(f"Size: {self.width}x{self.height}")
        print(f"Unique colors: {len(self.unique_colors)}\n")
        for c, info in sorted(self.unique_colors.items(), key=lambda x: x[1]["lumin"]):
            adj = info.get("adjacents", set())
            adj_str = ", ".join(sorted(adj)[:5]) if adj else "none"
            print(f"  {c}  L={info['lumin']:6.1f}  count={info['count']:6d}  samples={info['samples'][:2]}  touches=[{adj_str}]")

    def generate_texture(self, theme_colors, output_path):
        """Generate a recolored background PNG."""
        role_targets = {
            "outline_dark": theme_colors["background"]["outline_dark"],
            "shadow": theme_colors["background"]["shadow"],
            "fill": theme_colors["background"]["fill"],
            "edge_light": theme_colors["background"]["edge_light"],
        }
        role_map = self.classify_by_luminance()
        color_map = {}

        # Group by role for within-role interpolation
        ...
        for role, items in role_colors.items():
            ...
            for c, lum in items:
                ...
                adjusted = tuple(
                    max(0, min(255, int(target[i] * (0.82 + 0.36 * ratio))))
                    for i in range(3)
                ) + (target[3],)
        ...
        fallback = role_targets.get("fill", (24, 28, 34, 255))
        for (x, y), old in self.pixels.items():
            role = role_map.get(old, "fill")
            rgba = color_map.get(old, role_targets.get(role, fallback))
            idx = (y * self.width + x) * 4
            pixels_out[idx : idx + 4] = bytes(rgba)

        output_path = Path(output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        _save_rgba_png(output_path, self.width, self.height, bytes(pixels_out))
        return output_path


def _save_rgba_png(path, width, height, pixels):
    raw = bytearray()
    stride = width * 4
...
def generate_background_for_theme(pixel_map_path, template_path, theme_name, output_dir, resource_root=None):
    """High-level: load theme, recolor background, save output."""
    themes = load_themes(template_path)
    if theme_name not in themes:
        raise ValueError(f"Theme '{theme_name}' not found in {template_path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        print("  pixel_map.txt  - Pixel position file (x,y: #RRGGBB)")
        print("  template.json  - Theme palette template JSON")
        print("  theme_name     - Name of theme in template")
        print("  output_dir     - Output directory (default: generated_gui_textures)")
        sys.exit(1)
