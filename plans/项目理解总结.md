# Applied Storage Sorter — 项目理解总结

## 一、项目概览

**Applied Storage Sorter** 是一个 Minecraft NeoForge 模组（v1.21.1），为 AE2 (Applied Energistics 2) 提供存储治理能力。Mod ID 为 `appliedstoragesorter`，当前版本 1.0.0。
...
| 产物 | 路径 | 类型 |
|------|------|------|
| 技术运行日志 | `logs/appliedstoragesorter.log` | 文本 |
| Plan 报告 | `logs/appliedstoragesorter/plan-*.log` | 文本 |
| PlanAndMove 报告 | `logs/appliedstoragesorter/plan-and-move-*.log` | 文本 |
| Merge 报告 | `logs/appliedstoragesorter/merge-*.log` | 文本 |
| ME Dump | `dumps/appliedstoragesorter/me-dump-*.json` | JSON |
| Storage Analysis | `dumps/appliedstoragesorter/storage-analysis-*.json` | JSON |
